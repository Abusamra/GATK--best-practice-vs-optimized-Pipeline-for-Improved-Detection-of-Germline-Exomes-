...
java -Xmx20g -jar /app/Genome/picard-tools-2.2.1/picard.jar CollectQualityYieldMetrics \
      ...
      java -Xmx20g -jar /app/Genome/picard-tools-2.2.1/picard.jar CollectHsMetrics   I=Output_Germline_Aziz/$i.bam  O=Output_Germline_Aziz/${i}_collect_hs_metrics.txt R=../../genome/genome.fa  BAIT_INTERVALS=../../recalset/Probe.interval_list  TARGET_INTERVALS=../../recalset/Traget.interval_list PER_BASE_COVERAGE=Output_Germline_Aziz/${i}_base-coverage PER_TARGET_COVERAGE=Output_Germline_Aziz/${i}_targetcoverage
  
  #===================================================
  
rm Data/Intensities/BaseCalls/tt.txt
...
java -Xmx20g -jar /app/Genome/picard-tools-2.2.1/picard.jar CollectInsertSizeMetrics   I=Output_Germline_Aziz/$i.bam O=Output_Germline_Aziz/${i}_insert_size_metrics.txt   H=insert_size_histogram.pdf  M=0.5
...
sed -sn 8p Output_Germline_Aziz/${i}.rmdup.metrics.txt>Output_Germline_Aziz/${i}.rmdup.metrics2.txt
 ...
 echo '</h2> <br> <p align="center">Workflow : WES-Germline Pipeline</p> <p align="center">Report Date:<script> document.write(new Date().toLocaleDateString()); </script></p>' >>Alignment_Germline_Aziz/${i}_Report.html  
...
#------------------------------------------------------    
#---------Gender check-----------------------------
samtools idxstats Output_Germline_Aziz/$i.bam 2>/dev/null | awk '{ tmp=($3)/($2) ; printf"%s %0.5f\n", $1, tmp }' 2>/dev/null | head -23 > Output_Germline_Aziz/$i-autosomalcov.txt



min=$(echo $(sort -n -k 2 Output_Germline_Aziz/$i-autosomalcov.txt | head -n 1)| awk '{print $2}')

xcov=$(echo "scale=4; $(samtools idxstats Output_Germline_Aziz/$i.bam| grep "X" | cut -f 3)/$(samtools idxstats Output_Germline_Aziz/$i.bam | grep "X" | cut -f 2)" | bc)
ycov=$(echo "scale=4; $(samtools idxstats Output_Germline_Aziz/$i.bam | grep "Y" | cut -f 3)/$(samtools idxstats Output_Germline_Aziz/$i.bam | grep "Y" | cut -f 2)" | bc)
...
echo '<h3 align="left"> Software Versions </h3> <table> <tr> <th>Software</th> <th>bcl2fastq</th><th>BWA-mem (Aligner)</th> <th>Samtool</th> <th>Picard</th> <th>GATK-HaplotypeCaller </th> </tr> <tr> <td> Version </td> <td>v2-20 </td> <td> v-0.7.12</td> <td> v-1.2 </td>  <td> v-2.2.1 </td> <td>v-4.1.4</td></tr></table>'>> Alignment_Germline_Aziz/${i}_Report.html
...
# Generating Summary Report
 ...
 #java -Xmx20g -jar /app/Genome/picard-tools-2.2.1/picard.jar CollectIlluminaLaneMetrics       RUN_DIRECTORY= $(pwd)      OUTPUT_DIRECTORY=$(pwd)       OUTPUT_PREFIX=experiment1       READ_STRUCTURE=101B10B10B101B

#sed -sn 8,11p experiment1.illumina_lane_metrics> experiment2.illumina_lane_metrics.txt

#echo '<h2 align="left" style="color:black;"> Cluster Density</h2> <table border="1" ID="ReportTable"> <tr> <th>Cluster Density</th> <th>Lane</th>'  >> Alignment_Germline_Aziz/Summary.html
#awk 'BEGIN {}{print "<tr>";for(i=1;i<=NF;i++)print "<td>" $i"</td>";print "</tr>"} END{print "</table>"}' experiment2.illumina_lane_metrics.txt>> Alignment_Germline_Aziz/Summary.html
#===============================================================================
#===============================================================================
###### QC tarck
